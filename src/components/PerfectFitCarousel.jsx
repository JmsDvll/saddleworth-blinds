...
const PerfectFitCarousel = ({ showTitle = true, showDescription = true }) => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % perfectFitImages.length)
  }

  const prevSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + perfectFitImages.length) % perfectFitImages.length)
  }

  const currentImage = perfectFitImages[currentIndex]
      ...
      {showTitle && (
        <div className="p-6 text-center">
          <h3 className="text-2xl font-bold text-brand-gold mb-2">
            Perfect Fit Blinds Collection ({perfectFitImages.length} styles)
          </h3>
          {showDescription && (
            <p className="text-gray-300">
              Browse through our Perfect Fit blinds - no drilling required!
            </p>
          )}
        </div>
      )}
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-4 left-4 bg-black/70 text-white px-3 py-1 rounded">
          <span className="text-sm">
            {currentIndex + 1} / {perfectFitImages.length} - {currentImage.title}
          </span>
        </div>
      </div>
      ...
      {perfectFitImages.length > 1 && (
        <div className="p-4">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {perfectFitImages.map((image, index) => (
              <button
                key={index}
